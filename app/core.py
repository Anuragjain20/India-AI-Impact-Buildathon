"""
Core multi‑agent honeypot logic.

This file contains:
- Enums and dataclasses for session/intelligence
- In‑memory session store
- Multi‑agent implementations (detection, strategy, persona, intelligence)
- Orchestrator that coordinates everything
- Callback helper to report results back to GUVI
"""
...
from .config import GROQ_API_KEY, GROQ_MODEL, GUVI_CALLBACK_URL
...
Analyze this message and conversation context."""
        ...
        try:
            ...
            logger.error("Detection Agent error: %s", exc)
            return DetectionResult(
                scam_detected=False,
                confidence=0.0,
...
class StrategyAgent:
    ...
    Determines optimal conversation strategy and persona.
    """
...
What's the best strategy to continue this engagement and extract maximum intelligence?"""
        ...
        try:
            ...
            return StrategyPlan(
                recommended_persona=EngagementStrategy(
                    data.get("recommended_persona", "naive_victim"),
                ),
                conversation_phase=ConversationPhase(
                    data.get("conversation_phase", "initial"),
                ),
            ...
            logger.error("Strategy Agent error: %s", exc)
            return StrategyPlan(
...
class PersonaAgent:
    ...
    PERSONA_TEMPLATES: Dict[EngagementStrategy, str] = {
...
Respond with ONLY your message text. No explanations."""
        ...
        try:
            ...
            logger.error("Persona Agent error: %s", exc)
...
Return ONLY valid JSON:
{
  ...
  "agentNotes": "tactical analysis"
    ...
    async def extract(
        ...
        scammer_messages = [
            m for m in conversation_history if m.get("role") == "user"
        ]
        conv_text = "\n".join(
            f"[SCAMMER MESSAGE {idx + 1}]: {m['content']}"
            for idx, m in enumerate(scammer_messages)
        )
...
Extract all intelligence from these scammer messages."""
        ...
        try:
            ...
            logger.error("Intelligence Agent error: %s", exc)
            return {
                "bankAccounts": [],
                "upiIds": [],
                "phishingLinks": [],
                ...
                "organizationsMentioned": [],
                "amountsRequested": [],
                "agentNotes": f"Extraction error: {str(exc)}",
            }
...
class OrchestratorAgent:
    ...
    async def process_message(
        ...
        """
        logger.info(
            "🎯 ORCHESTRATOR: Processing message for session %s",
        ...
        logger.info(
            "   Scam Detected=%s, Conf=%.2f, Type=%s",
            detection_result.scam_detected,
            detection_result.confidence,
            detection_result.scam_type.value,
        )
        ...
        if not detection_result.scam_detected or detection_result.confidence < 0.5:
            logger.info(
                "Not detected as scam with high confidence - returning neutral response",
            )
    ...
    @staticmethod
    def _merge_intelligence(current: Intelligence, new: Dict[str, Any]) -> None:
        field_mapping = {
    ...
    @staticmethod
    def should_send_callback(session_state: SessionState) -> bool:
        """Determine if we should send final callback."""
        if session_state.callback_sent:
            return False

        has_critical_intel = (
            bool(session_state.intelligence.bank_accounts)
            or bool(session_state.intelligence.upi_ids)
            or bool(session_state.intelligence.phishing_links)
            or bool(session_state.intelligence.phone_numbers)
        )
        ...
        return (
            (has_critical_intel and sufficient_engagement)
            or high_engagement
            or session_state.engagement_quality >= 0.6
        )
...
async def send_guvi_callback(session_state: SessionState) -> None:
    """Send final intelligence to GUVI evaluation endpoint."""
    if session_state.callback_sent:
        logger.info("Callback already sent for session %s", session_state.session_id)
        return
    ...
    payload = {
        ...
        "agentNotes": (
            "; ".join(session_state.agent_notes)
            if session_state.agent_notes
            else "Multi-agent system engaged successfully"
        ),
    }

    logger.info("📤 SENDING FINAL CALLBACK TO GUVI")
    logger.info(json.dumps(payload, indent=2))

    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.post(GUVI_CALLBACK_URL, json=payload)
        logger.info("Callback response [%s]: %s", response.status_code, response.text)
